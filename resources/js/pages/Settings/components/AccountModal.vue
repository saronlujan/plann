    ...
    <Dialog v-model:open="dialogOpen">
        <DialogContent class="sm:max-w-md">
            <DialogHeader>
                <DialogTitle>{{
                    isEdit
                        ? $t('settings.accounts.modal.edit_title')
                        : $t('settings.accounts.modal.create_title')
                }}</DialogTitle>
                <DialogDescription>{{
                    $t('settings.accounts.modal.description')
                }}</DialogDescription>
            </DialogHeader>
            ...
            <Form class="gap-3" @submit.prevent="submit">
                <FormGroup>
                    <FormLabel for="acc-name">{{
                        $t('settings.accounts.modal.name_label')
                    }}</FormLabel>
                    <Input
                        id="acc-name"
                        v-model="form.name"
                        :placeholder="$t('settings.accounts.modal.name_placeholder')"
                    />
                ...
                <FormGroup>
                    <FormLabel for="acc-kind">{{
                        $t('settings.accounts.modal.kind_label')
                    }}</FormLabel>
                ...
                <FormGroup>
                    <FormLabel for="acc-currency">{{
                        $t('settings.accounts.modal.currency_label')
                    }}</FormLabel>
                    <Select v-model="form.currency_id">
                        <SelectTrigger id="acc-currency">
                            <SelectValue
                                :placeholder="$t('settings.accounts.modal.currency_placeholder')"
                            />
                        </SelectTrigger>
                        <SelectContent>
                            <SelectItem
                ...
                <FormGroup v-if="!isCard">
                    <FormLabel for="acc-balance">{{
                        $t('settings.accounts.modal.balance_label')
                    }}</FormLabel>
                    <Input
                        id="acc-balance"
                        ...
                        type="number"
                        step="0.01"
                        :placeholder="$t('settings.accounts.modal.balance_placeholder')"
                    />
                ...
                <template v-if="isCard">
                    <FormGroup>
                        <FormLabel for="acc-limit">{{
                            $t('settings.accounts.modal.credit_limit_label')
                        }}</FormLabel>
                        <Input
                            id="acc-limit"
                            ...
                            type="number"
                            step="0.01"
                            :placeholder="$t('settings.accounts.modal.credit_limit_placeholder')"
                        />
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <FormGroup>
                            <FormLabel for="acc-closing">{{
                                $t('settings.accounts.modal.closing_day_label')
                            }}</FormLabel>
                            <Input
                                id="acc-closing"
                                ...
                                min="1"
                                max="31"
                                :placeholder="$t('settings.accounts.modal.day_placeholder')"
                            />
                        ...
                        <FormGroup>
                            <FormLabel for="acc-due">{{
                                $t('settings.accounts.modal.due_day_label')
                            }}</FormLabel>
                            <Input
                                id="acc-due"
