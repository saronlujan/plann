...
/**
 * Every filter change is a fresh server render: projecting a recurrence is the
 * server's job, so the browser never holds a second copy of the rules.
 *
 * An empty string means "drop this one" rather than "send it blank", which is
 * what keeps the URL down to what is actually in force.
 */
function applyFilters(patch: Record<string, string>): void {
    const merged: Record<string, string> = {
        scale: props.filters.scale,
        date: props.filters.date,
        search: props.filters.search,
        order: props.filters.order,
        from: props.filters.custom_range ? props.filters.from : '',
        to: props.filters.custom_range ? props.filters.to : '',
        ...patch,
    };

    const query = Object.fromEntries(
        Object.entries(merged).filter(([, value]) => value !== '' && value !== 'date_desc'),
    );

    router.get(transactions.index().url, query, {
        preserveScroll: true,
        preserveState: true,
        replace: true,
    });
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead>
                                    <SortableHeader
                                        :label="$t('transactions.columns.transaction')"
                                        :orders="['name_asc', 'name_desc']"
                                        :current="filters.order"
                                        @sort="(order) => applyFilters({ order })"
                                    />
                                </TableHead>
                                <TableHead></TableHead>
